...
makeSuite('StakedREX V2. Redeem', (testEnv: TestEnv) => {
  it('Reverts trying to redeem 0 amount', async () => {
    ...
    await expect(
      StakedREXV3.connect(staker.signer).redeem(staker.address, amount)
  ...
  it('User 1 stakes 50 REX', async () => {
    ...
    await waitForTx(await rexToken.connect(staker.signer).approve(StakedREXV3.address, amount));
    await waitForTx(await StakedREXV3.connect(staker.signer).stake(staker.address, amount));
  });
  ...
  it('User 1 activates the cooldown, but is not able to redeem before the COOLDOWN_SECONDS passed', async () => {
    ...
    const startedCooldownAt = new BigNumber(
      await (await StakedREXV3.stakersCooldowns(staker.address)).toString()
    );
  ...
  it('User 1 activates the cooldown again, and tries to redeem a bigger amount that he has staked, receiving the balance', async () => {
    ...
    const StakedREXBalanceBefore = (await StakedREXV3.balanceOf(staker.address)).toString();
    await StakedREXV3.connect(staker.signer).redeem(staker.address, amount);
    const REXBalanceAfter = new BigNumber((await rexToken.balanceOf(staker.address)).toString());
    const StakedREXBalanceAfter = (await StakedREXV3.balanceOf(staker.address)).toString();
  ...
  it('User 4 stakes 50 REX, activates the cooldown and redeems half of the amount', async () => {
    ...
    const REXBalanceBefore = new BigNumber((await rexToken.balanceOf(staker.address)).toString());
    await StakedREXV3.connect(staker.signer).redeem(
      staker.address,
