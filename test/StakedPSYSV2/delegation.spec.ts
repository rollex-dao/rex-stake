...
makeSuite('StakedPSYSV2. Power Delegations', (testEnv: TestEnv) => {
  ...
  xit('ZERO_ADDRESS tries to delegate voting power to user1 but delegatee should still be ZERO_ADDRESS', async () => {
    ...
    await waitForTx(await StakedPSYSV2.connect(zeroUser).delegateByType(user1.address, '0'));

    const delegatee = await StakedPSYSV2.getDelegateeByType(ZERO_ADDRESS, '0');

    expect(delegatee.toString()).to.be.equal(ZERO_ADDRESS);
  });

  it('User 1 tries to delegate voting power to user 2', async () => {
    const { users, StakedPSYSV2 } = testEnv;

    await waitForTx(
      await StakedPSYSV2.connect(users[1].signer).delegateByType(users[2].address, '0')
    );

    const delegatee = await StakedPSYSV2.getDelegateeByType(users[1].address, '0');
  ...
  it('User 1 tries to delegate proposition power to user 3', async () => {
    ...
    await waitForTx(
      await StakedPSYSV2.connect(users[1].signer).delegateByType(users[3].address, '1')
    );

    const delegatee = await StakedPSYSV2.getDelegateeByType(users[1].address, '1');
  ...
  it('User1 tries to delegate voting power to ZERO_ADDRESS but delegator should remain', async () => {
    ...
    // Stake
    await waitForTx(
      await psysToken.connect(user.signer).approve(StakedPSYSV2.address, psysBalance)
    );
    await waitForTx(await StakedPSYSV2.connect(user.signer).stake(user.address, psysBalance));

    // Track current power
    const priorPowerUser = await StakedPSYSV2.getPowerCurrent(user.address, '0');
    const priorPowerUserZeroAddress = await StakedPSYSV2.getPowerCurrent(ZERO_ADDRESS, '0');
  ...
  it('User 1 stakes 2 PSYS; checks voting and proposition power of user 2 and 3', async () => {
    ...
    // Stake
    await waitForTx(
      await psysToken.connect(user1.signer).approve(StakedPSYSV2.address, psysBalance)
    );
    const tx = await waitForTx(
      await StakedPSYSV2.connect(user1.signer).stake(user1.address, psysBalance)
    );

    const stkPSYSBalanceAfterMigration = await StakedPSYSV2.balanceOf(user1.address);

    firstActionBlockNumber = tx.blockNumber;

    const user1PropPower = await StakedPSYSV2.getPowerCurrent(user1.address, '0');
    const user1VotingPower = await StakedPSYSV2.getPowerCurrent(user1.address, '1');

    const user2VotingPower = await StakedPSYSV2.getPowerCurrent(user2.address, '0');
    const user2PropPower = await StakedPSYSV2.getPowerCurrent(user2.address, '1');

    const user3VotingPower = await StakedPSYSV2.getPowerCurrent(user3.address, '0');
    const user3PropPower = await StakedPSYSV2.getPowerCurrent(user3.address, '1');
  ...
  it('User 2 stakes 2 LEND; checks voting and proposition power of user 2', async () => {
    ...
    // Stake
    await waitForTx(
      await psysToken.connect(user2.signer).approve(StakedPSYSV2.address, psysBalance)
    );
    await waitForTx(await StakedPSYSV2.connect(user2.signer).stake(user2.address, psysBalance));
  ...
  it('User 3 migrates 2 LEND; checks voting and proposition power of user 3', async () => {
    ...
    // Stake
    await waitForTx(
      await psysToken.connect(user3.signer).approve(StakedPSYSV2.address, psysBalance)
    );
    await waitForTx(await StakedPSYSV2.connect(user3.signer).stake(user3.address, psysBalance));
  ...
  it('User 2 delegates voting and prop power to user 3', async () => {
    ...
    await waitForTx(await StakedPSYSV2.connect(user2.signer).delegate(user3.address));
  ...
  it('User 1 removes voting and prop power to user 2 and 3', async () => {
    ...
    await waitForTx(await StakedPSYSV2.connect(user1.signer).delegate(user1.address));
  ...
  it('Checks the delegation at the block of the first action', async () => {
    ...
    const user1VotingPower = await StakedPSYSV2.getPowerAtBlock(
      user1.address,
      firstActionBlockNumber,
      '0'
    );
    const user1PropPower = await StakedPSYSV2.getPowerAtBlock(
      user1.address,
      firstActionBlockNumber,
      '1'
    );

    const user2VotingPower = await StakedPSYSV2.getPowerAtBlock(
      user2.address,
      firstActionBlockNumber,
      '0'
    );
    const user2PropPower = await StakedPSYSV2.getPowerAtBlock(
      user2.address,
      firstActionBlockNumber,
      '1'
    );

    const user3VotingPower = await StakedPSYSV2.getPowerAtBlock(
      user3.address,
      firstActionBlockNumber,
      '0'
    );
    const user3PropPower = await StakedPSYSV2.getPowerAtBlock(
      user3.address,
      firstActionBlockNumber,
      '1'
    );
  ...
  it('Ensure that getting the power at the current block is the same as using getPowerCurrent', async () => {
    ...
    const votingPowerAtPreviousBlock = await StakedPSYSV2.getPowerAtBlock(
      user1.address,
      currentBlock - 1,
      '0'
    );
    const votingPowerCurrent = await StakedPSYSV2.getPowerCurrent(user1.address, '0');

    const propPowerAtPreviousBlock = await StakedPSYSV2.getPowerAtBlock(
      user1.address,
      currentBlock - 1,
      '1'
    );
    const propPowerCurrent = await StakedPSYSV2.getPowerCurrent(user1.address, '1');
  ...
  it("Checks you can't fetch power at a block in the future", async () => {
    ...
    await expect(
      StakedPSYSV2.getPowerAtBlock(user1.address, currentBlock + 1, '0')
    ).to.be.revertedWith('INVALID_BLOCK_NUMBER');
    await expect(
      StakedPSYSV2.getPowerAtBlock(user1.address, currentBlock + 1, '1')
    ).to.be.revertedWith('INVALID_BLOCK_NUMBER');
  });
  ...
  it('User 1 transfers value to himself. Ensures nothing changes in the delegated power', async () => {
    ...
    const user1VotingPowerBefore = await StakedPSYSV2.getPowerCurrent(user1.address, '0');
    const user1PropPowerBefore = await StakedPSYSV2.getPowerCurrent(user1.address, '1');

    const balance = await StakedPSYSV2.balanceOf(user1.address);

    await waitForTx(await StakedPSYSV2.connect(user1.signer).transfer(user1.address, balance));

    const user1VotingPowerAfter = await StakedPSYSV2.getPowerCurrent(user1.address, '0');
    const user1PropPowerAfter = await StakedPSYSV2.getPowerCurrent(user1.address, '1');
  ...
  it('User 1 delegates voting power to User 2 via signature', async () => {
    ...
    const user2VotPower = await StakedPSYSV2.getPowerCurrent(user2.address, '1');
    const expectedVotingPower = (await StakedPSYSV2.getPowerCurrent(user1.address, '1')).add(
      user2VotPower
    );

    // Check prior delegatee is still user1
    const priorDelegatee = await StakedPSYSV2.getDelegateeByType(user1.address, '0');
    ...
    const nonce = (await StakedPSYSV2._nonces(user1.address)).toString();
    ...
    const tx = await StakedPSYSV2.connect(user1.signer).delegateByTypeBySig(
      user2.address,
      '0',
      nonce,
      expiration,
    ...
    const delegatee = await StakedPSYSV2.getDelegateeByType(user1.address, '0');
  ...
  it('User 1 delegates proposition to User 3 via signature', async () => {
    ...
    const user3PropPower = await StakedPSYSV2.getPowerCurrent(user3.address, '1');
    const expectedPropPower = (await StakedPSYSV2.getPowerCurrent(user1.address, '1')).add(
      user3PropPower
    );

    // Check prior proposition delegatee is still user1
    const priorDelegatee = await StakedPSYSV2.getDelegateeByType(user1.address, '1');
    ...
    const delegatee = await StakedPSYSV2.getDelegateeByType(user1.address, '1');
    expect(delegatee.toString()).to.be.equal(user3.address, 'Delegatee should be user 3');

    const user3PropositionPower = await StakedPSYSV2.getPowerCurrent(user3.address, '1');
  ...
  it('User 2 delegates all to User 4 via signature', async () => {
    ...
    await waitForTx(await StakedPSYSV2.connect(user2.signer).delegate(user2.address));

    // Calculate expected powers
    const user4PropPower = await StakedPSYSV2.getPowerCurrent(user4.address, '1');
    const expectedPropPower = (await StakedPSYSV2.getPowerCurrent(user2.address, '1')).add(
      user4PropPower
    );

    const user1VotingPower = await StakedPSYSV2.balanceOf(user1.address);
    const user4VotPower = await StakedPSYSV2.getPowerCurrent(user4.address, '0');
    const user2ExpectedVotPower = user1VotingPower;
    const user4ExpectedVotPower = (await StakedPSYSV2.getPowerCurrent(user2.address, '0'))
      .add(user4VotPower)
      .sub(user1VotingPower); // Delegation does not delegate votes others from other delegations

    // Check prior proposition delegatee is still user1
    const priorPropDelegatee = await StakedPSYSV2.getDelegateeByType(user2.address, '1');
    expect(priorPropDelegatee.toString()).to.be.equal(
      user2.address,
      'expected proposition delegatee to be user1'
    );

    const priorVotDelegatee = await StakedPSYSV2.getDelegateeByType(user2.address, '0');
    ...
    const nonce = (await StakedPSYSV2._nonces(user2.address)).toString();
    const expiration = MAX_UINT_AMOUNT;
    ...
    // Transmit tx via delegateByTypeBySig
    const tx = await StakedPSYSV2.connect(user2.signer).delegateBySig(
      user4.address,
      nonce,
      expiration,
      v,
    ...
    // Check internal state matches events
    const propDelegatee = await StakedPSYSV2.getDelegateeByType(user2.address, '1');
    expect(propDelegatee.toString()).to.be.equal(
      user4.address,
      'Proposition delegatee should be user 4'
    );

    const votDelegatee = await StakedPSYSV2.getDelegateeByType(user2.address, '0');
    expect(votDelegatee.toString()).to.be.equal(user4.address, 'Voting delegatee should be user 4');

    const user4PropositionPower = await StakedPSYSV2.getPowerCurrent(user4.address, '1');
    expect(user4PropositionPower).to.be.equal(
      expectedPropPower,
      'Delegatee should have propostion power from user 2'
    );
    const user4VotingPower = await StakedPSYSV2.getPowerCurrent(user4.address, '0');
    expect(user4VotingPower).to.be.equal(
      user4ExpectedVotPower,
      'Delegatee should have votinh power from user 2'
    );

    const user2PropositionPower = await StakedPSYSV2.getPowerCurrent(user2.address, '1');
  ...
  it('User 1 should not be able to delegate with bad signature', async () => {
    ...
    // Transmit message via delegateByTypeBySig
    await expect(
      StakedPSYSV2.connect(user1.signer).delegateByTypeBySig(
        user2.address,
        '0',
        nonce,
        expiration,
  ...
  it('User 2 should not be able to delegate all with bad signature', async () => {
    ...
    await expect(
      StakedPSYSV2.connect(user2.signer).delegateBySig(user4.address, nonce, expiration, '0', r, s)
    ).to.be.revertedWith('INVALID_SIGNATURE');
  });
  ...
  it('User 2 should not be able to delegate all with bad nonce', async () => {
    ...
    // Transmit tx via delegateByTypeBySig
    await expect(
      StakedPSYSV2.connect(user2.signer).delegateBySig(user4.address, nonce, expiration, v, r, s)
    ).to.be.revertedWith('INVALID_NONCE');
  });
  ...
  it('Correct proposal and voting snapshotting on double action in the same block', async () => {
    ...
    // Reset delegations
    await waitForTx(await StakedPSYSV2.connect(user1.signer).delegate(user1.address));
    await waitForTx(await StakedPSYSV2.connect(receiver.signer).delegate(receiver.address));

    const user1PriorBalance = await StakedPSYSV2.balanceOf(user1.address);
    const receiverPriorPower = await StakedPSYSV2.getPowerCurrent(receiver.address, '0');
    const user1PriorPower = await StakedPSYSV2.getPowerCurrent(user1.address, '0');

    // Deploy double transfer helper
    const doubleTransferHelper = await deployDoubleTransferHelper(StakedPSYSV2.address);

    await waitForTx(
      await StakedPSYSV2.connect(user1.signer).transfer(
    ...
    const receiverCurrentPower = await StakedPSYSV2.getPowerCurrent(receiver.address, '0');
    const user1CurrentPower = await StakedPSYSV2.getPowerCurrent(user1.address, '0');
