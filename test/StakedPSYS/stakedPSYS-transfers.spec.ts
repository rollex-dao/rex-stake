...
makeSuite('StakedREX. Transfers', (testEnv: TestEnv) => {
  ...
  it('User 1 transfers 50 SREX to User 5', async () => {
    const { StakedREX, users } = testEnv;
    const amount = ethers.utils.parseEther('50').toString();
    const sender = users[1];
    const receiver = users[5];

    await compareRewardsAtTransfer(StakedREX, sender, receiver, amount, true, false);
  });

  it('User 5 transfers 50 SREX to himself', async () => {
    const { StakedREX, users } = testEnv;
    const amount = ethers.utils.parseEther('50');
    const sender = users[5];
    await compareRewardsAtTransfer(StakedREX, sender, sender, amount, true, true);
  });

  it('User 5 transfers 50 SREX to user 2, with rewards not enabled', async () => {
    const { StakedREX, rexToken, users } = testEnv;
    const amount = ethers.utils.parseEther('50');
    const sender = users[5];
    ...
    await compareRewardsAtTransfer(StakedREX, sender, receiver, amount, false, false, assetConfig);
  });

  it('User 4 stakes and transfers 50 SREX to user 2, with rewards not enabled', async () => {
    const { StakedREX, rexToken, users } = testEnv;
    const amount = ethers.utils.parseEther('50');
    ...
    const actions = () => [
      rexToken.connect(sender.signer).approve(StakedREX.address, amount),
      StakedREX.connect(sender.signer).stake(sender.address, amount),
    ];

    await compareRewardsAtAction(StakedREX, sender.address, actions, false, assetConfig);
    await compareRewardsAtTransfer(StakedREX, sender, receiver, amount, false, false, assetConfig);
  });
  it('Activate cooldown of User2, transfer entire amount from User2 to User3, cooldown of User2 should be reset', async () => {
    ...
    const amount = await StakedREX.balanceOf(sender.address);
    ...
    await StakedREX.connect(sender.signer).cooldown();
    const cooldownActivationTimestamp = await (await timeLatest()).toString();

    const cooldownTimestamp = await StakedREX.stakersCooldowns(sender.address);
    ...
    const cooldownTimestampAfterTransfer = await (
      await StakedREX.stakersCooldowns(sender.address)
    ).toString();
  ...
  it('Transfer balance from User 3 to user 2 cooldown  of User 2 should be reset if User3 cooldown expired', async () => {
    ...
    await rexToken.connect(receiver.signer).approve(StakedREX.address, amount);
    await StakedREX.connect(receiver.signer).stake(receiver.address, amount);
    await StakedREX.connect(receiver.signer).cooldown();
    const receiverCooldown = await StakedREX.stakersCooldowns(sender.address);
    ...
    // Transfer staked rex from sender to receiver, it will also transfer the cooldown status from sender to the receiver
    await compareRewardsAtTransfer(StakedREX, sender, receiver, amount, false, false, assetConfig);

    // Receiver cooldown should be set to zero
    const stakerCooldownTimestampBefore = await StakedREX.stakersCooldowns(receiver.address);
  ...
  it('Transfer balance from User 3 to user 2, cooldown of User 2 should be the same if User3 cooldown is less than User2 cooldown', async () => {
    ...
    await StakedREX.connect(receiver.signer).cooldown();
    const receiverCooldown = await (await StakedREX.stakersCooldowns(receiver.address)).toString();
    ...
    const receiverCooldownAfterTransfer = await (
      await StakedREX.stakersCooldowns(receiver.address)
    ).toString();
